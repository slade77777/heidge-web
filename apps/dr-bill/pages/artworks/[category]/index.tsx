...
].reduce((acc, element) => ({...acc, [element.title.replaceAll(' ', '-').toLowerCase()]: element}), []);

const artworks = Array.from(Array(29).keys()).map((_, i) => (
  {
    title: 'Coming and Going',
    description: 'Good vibrations is an original Op Art piece. It is a two tile tessellation where one tile...',
    image: [{src: `https://res.cloudinary.com/suborg/image/upload/v1647255699/DrBill/1920/plotter-${i + 1}.png`}],
  }
))
const CategoryPage: NextPage = () => {
  ...
  return (
    <ArtworksByCategory artworks={artworks} category={category}/>
  );
};
